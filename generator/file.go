...
func FormatGoFile(path string) error {
	if s := strings.Split(path, "."); len(s) == 0 || s[len(s)-1] != "go" {
		return nil
	}
	var cmd *exec.Cmd
...
// CreateFile 创建文件
func CreateFile(path string, data []byte, mode uint32) error {
	return ioutil.WriteFile(path, data, os.FileMode(mode))
}

// CreateAndFormat 创建文件并格式化
func CreateAndFormat(path string, data []byte, mode uint32) error {
	if err := CreateFile(path, data, mode); err != nil {
		return err
	}
...
func IsExists(path string) bool {
	...
	panic("file is Exists error" + err.Error())
}
